...
-- Quad Button default padding and dimensions
local btnPadW, btnPadH = 32, 16
local btnDefW, btnDefH = 180, 40
...
if not ButtonQuad then
	...
	function mt:draw ()
		local sx, sy = btnDefW, btnDefH--self:getSize()
		local x0 = self.x - sx/2
		local y0 = self.y - sy/2
		local x1 = x0 + sx
		local y1 = y0 + sy
		...
		if cx >= x0 and cx <= x1 and cy >= y0 and cy <= y1 then
			...
			if click then
				...
				cb = cb + 0.1
				-- Click action
				if click and type(self.action) == "function" then
					self.action()
				end
			end
		end
...
--Floors
if config.floors then
	for _, v in spairs(config.floors) do
		local color = "#450101"
...
-- local fgSpace = {}
-- local topY = 115
-- if fuelAtmo.tanks and #fuelAtmo.tanks > 0 then
-- 	local aSpacing = 200
...
-- 	end
-- end
-- if fuelSpace.tanks and #fuelSpace.tanks > 0 then
-- 	local aSpacing = 200
...
if not config.settingsActive and not config.setBaseActive and not config.shutDown then
	...
	-- drawFree(fgAtmo)
	-- drawFree(fgSpace)
	if type(stats) == 'table' and stats.data then
		-- logMessage('Stats: '..tostring(stats.data))
		StatsContainer()
